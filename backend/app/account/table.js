...
class AccountTable {
  ...
  static getAccount({ usernameHash }) {
    return new Promise((resolve, reject) => {
      pool.query(
        'SELECT id,"sessionId","passwordHash",balance FROM account WHERE "usernameHash"= $1',
        [usernameHash],
        (error, response) => {
          if (error) return reject(error);
...
// AccountTable.updateBalance({ accountId: 1, value: 1500 })
//   .then(() => console.log("update balance"))
//   .catch((e) => console.error(e));

//node app/account/table.js
